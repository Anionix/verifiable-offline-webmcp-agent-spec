...
updated_at: "2026-08-29T13:52:59.000Z"
version: "0.1.0"
...
> **公開状態 / Public status:** これは設計仕様と参照実装であり、実働製品ではありません。訪日旅行者向けホテルデモは、[ChatGPT Sites](https://kyoto-booking-retry-proof.anionix.chatgpt.site)と[Vercel](https://kyoto-booking-retry-proof.vercel.app)で一般公開しています。現在のVercel本番配置`dpl_AdzeHw7CgM3sbsZBVutZZbLLbeAK`はソースコミット`c8be388d8047472ef7d6ad69656255adb5903e37`に結び付き、匿名評価ファイルは192件のNode.js試験を報告します。対応する隔離Chrome実測では、ネイティブWebMCPの四機能、発見時の作用ゼロ、再送前の状態確認、試行2回・予約1件・処理開始1回・同じ確認番号を確認済みです。[Devpostの公開ページ](https://devpost.com/software/project-y79pb23hj1mz)も版12として現在の192件と現行公開証拠へ更新済みです。審査向け[YouTube動画](https://youtu.be/tdSvJw4ghX8)も一般公開済みです。過去の153件スナップショットは[`metadata/devpost-public-readback.json`](metadata/devpost-public-readback.json)に履歴として残し、現在の証拠とは混ぜません。実ホテル予約、決済、取消は一切行いません。通知デモの既存証拠は変更せず保存しています。
...
The current release candidate passes 192 Node tests, TypeScript checking, four-tool discovery and execution in the in-app browser, a WebMCP preparation that enables the separate human confirmation button, expiry recovery with a fresh approval window, arbitrary-input reload restoration, production-build offline reload, and 320/375/390/768-pixel overflow checks. The booking tests also count the physical IndexedDB booking rows and find exactly one. The public Vercel release file reports the same 192 tests, and the current Devpost page was updated to the same count.

Public ChatGPT Sites version 12 is a separate publication surface. It uses source commit `f832cc611ed43613035a8735ca97d4bc1a0a8efc`, deployment `appgdep_6a92e425bbdc8191ac9ca35d524f61a0`, and carried-forward browser proof from the same functional digest; it is not the source of the current native WebMCP evidence.

The current [Vercel hotel demo](https://kyoto-booking-retry-proof.vercel.app) is production deployment `dpl_AdzeHw7CgM3sbsZBVutZZbLLbeAK`, published from source commit `c8be388d8047472ef7d6ad69656255adb5903e37` and `READY` at `2026-08-30T13:47:14.754Z`. Its [deployment-specific URL](https://kyoto-booking-retry-proof-jnkzuh7r0-aniotajp-1978s-projects.vercel.app) and public alias serve the same WebMCP evaluation file; anonymous HTTP readback matches the local bytes. The fresh supported Chrome run on this deployment exposed `document.modelContext` and exactly the four intended tools, observed zero discovery effects, and verified native status recovery before the visible retry. The final result was `RETRY_RECOGNIZED`, two attempts, one booking, one effect start, and confirmation `FKR-08DFFA4C4F`.
