...
> **公開状態 / Public status:** これは設計仕様と参照実装であり、実働製品ではありません。訪日旅行者向けホテルデモは、[ChatGPT Sites](https://kyoto-booking-retry-proof.anionix.chatgpt.site)と[Vercel](https://kyoto-booking-retry-proof.vercel.app)で一般公開しています。提出先のSitesは第14版（ソースコミット`2fbbf1b714ca660ef1681239b638205a9835f7c5`）、Vercel本番は配置`dpl_BetejQ7wQwwouGcyC6HW588EiqK8`（ソースコミット`adeaf3f549aa7379acacbc6960eae4c11bbc6ba2`）です。両方の匿名評価ファイルは194件のNode.js試験と意図した四機能を報告します。[Devpostの公開ページ](https://devpost.com/software/project-y79pb23hj1mz)は版14として提出先Sitesと現行Vercel配備の説明へ更新済みです。現行配備の一意な別名でネイティブWebMCPの再試行証拠も実行済みです。過去の153件スナップショットは[`metadata/devpost-public-readback.json`](metadata/devpost-public-readback.json)に履歴として残し、現在の証拠とは混ぜません。実ホテル予約、決済、取消は一切行いません。通知デモの既存証拠は変更せず保存しています。
...
The current [Vercel hotel demo](https://kyoto-booking-retry-proof.vercel.app) is production deployment `dpl_BetejQ7wQwwouGcyC6HW588EiqK8`, published from source commit `adeaf3f549aa7379acacbc6960eae4c11bbc6ba2` and recorded `READY`. Its public alias serves the WebMCP evaluation file with anonymous HTTP 200 and 194-test release evidence. The managed browser proof is recorded separately; its fresh run used the unique alias of this same deployment after the READY boundary.
