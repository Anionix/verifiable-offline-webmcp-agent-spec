...
sites_version_12_event_uuid_v7: "01a04dcb-8678-79b7-a47d-aa87346680c3"
...
> **公開状態 / Public status:** これは設計仕様と参照実装であり、実働製品ではありません。訪日旅行者向けホテルデモは、[ChatGPT Sites](https://kyoto-booking-retry-proof.anionix.chatgpt.site)と[Vercel](https://kyoto-booking-retry-proof.vercel.app)で一般公開しています。提出先のSitesは第14版（ソースコミット`2fbbf1b714ca660ef1681239b638205a9835f7c5`）、Vercel本番の確認対象は配置`dpl_HWJVg4uCgFEaq9N2f5kvXwLjvK2E`（ソースコミット`2d5abd679893ec7dff36758925477999424c3cc7`）です。両方の匿名評価ファイルは194件のNode.js試験と意図した四機能を報告します。[Devpostの公開ページ](https://devpost.com/software/project-y79pb23hj1mz)は版16として提出先SitesとVercel配備の説明へ更新済みです。公開HTTPS別名でネイティブWebMCPの再試行証拠も実行済みです。過去の153件スナップショットは[`metadata/devpost-public-readback.json`](metadata/devpost-public-readback.json)に履歴として残し、現在の証拠とは混ぜません。実ホテル予約、決済、取消は一切行いません。通知デモの既存証拠は変更せず保存しています。
...
- `metadata/hotel-public-release-readback.json` — current Vercel source, deployment, anonymous HTTP readback, and exact public test count
...
- `metadata/public-release-alignment-readback.json` — separate anonymous Sites/Vercel and authenticated Devpost readbacks bound to one current description
...
The four 1.0.0 governance slices bind security, replay, operational-quality, browser-observation, and formal-evidence claims. The catalog contains **67 implemented and automated tests, with no partial or specification-only records**. Critical commit tools remain inside policy control; replay requires six fresh checks; an external effect needs independent readback rather than a tool claim alone; and six hard mathematical gates stop unsafe capacity, calibration, probability, or provenance inputs. Production operational quality remains `UNMEASURED`; the hotel demo has a `PASS` for the recorded native WebMCP configuration, while broader native conformance is not claimed. See [`src/typescript/governance/security-boundary.ts`](src/typescript/governance/security-boundary.ts), [`src/typescript/governance/replay-verification.ts`](src/typescript/governance/replay-verification.ts), [`src/typescript/governance/slo-gates.ts`](src/typescript/governance/slo-gates.ts), [`metadata/final-verification.json`](metadata/final-verification.json), and [`docs/test-catalog.md`](docs/test-catalog.md).
...
The current release candidate passes 194 Node tests, TypeScript checking, four-tool discovery and execution in the in-app browser, a WebMCP preparation that enables the separate human confirmation button, expiry recovery with a fresh approval window, arbitrary-input reload restoration, production-build offline reload, and 320/375/390/768-pixel overflow checks. The booking tests also count the physical IndexedDB booking rows and find exactly one. The public Sites and Vercel release files report the same 194 tests, and the current Devpost page was updated to the same count.

Public ChatGPT Sites version 14 is the submitted publication surface. It uses source commit `2fbbf1b714ca660ef1681239b638205a9835f7c5`; its public evaluation file is contract-ready and reports the same 194-test, four-tool boundary as Vercel. The current native WebMCP evidence remains a separately bounded record tied to the Vercel target below.

The recorded [Vercel hotel demo](https://kyoto-booking-retry-proof.vercel.app) target is production deployment `dpl_HWJVg4uCgFEaq9N2f5kvXwLjvK2E`, published from source commit `2d5abd679893ec7dff36758925477999424c3cc7` and recorded `READY` at `2026-08-31T10:31:51.086Z`. Its public alias serves the WebMCP evaluation file with anonymous HTTP 200 and 194-test release evidence. The managed browser proof is recorded separately; its fresh run used the canonical public HTTPS alias after the READY boundary and completed final observations at `2026-08-31T10:45:19.736Z`. The deployment-specific alias was independently read but redirected to sign-in in that isolated browser.
...
The current YouTube thumbnail is the owner-approved source artifact [`docs/assets/devpost-hotel-thumbnail.png`](docs/assets/devpost-hotel-thumbnail.png), and its public JPEG readback is verified at `https://i.ytimg.com/vi/tdSvJw4ghX8/maxresdefault.jpg` with SHA-256 `3ad391ba3efc35f61566299848dd1a3deeff05efc1e263a8dbfd73af293c8792`; the earlier Canva file-chooser timeout remains an explicit historical fallback. This is separate from Devpost: the visible page title, heading, Open Graph title, and Open Graph image of its [public project page](https://devpost.com/software/project-y79pb23hj1mz) identify `Kyoto Booking Retry Proof` and the uploaded hotel screenshot. Devpost version 16 now states the submitted Sites version 14, the recorded Vercel deployment, 194-test release evidence, exact four-tool contract, and status recovery before retry. Devpost submission `1158722` has status `Submitted`; the project update was read back after version 16 without creating a new submission receipt. Keyboard traversal and screen-reader behavior remain `INCONCLUSIVE`; the tested native WebMCP configuration is recorded as `PASS`, without claiming broad conformance beyond that configuration. See [`metadata/hotel-booking-verification.json`](metadata/hotel-booking-verification.json), [`metadata/hotel-public-release-readback.json`](metadata/hotel-public-release-readback.json), [`metadata/public-release-alignment-readback.json`](metadata/public-release-alignment-readback.json), [`metadata/hotel-native-webmcp-reconciliation.json`](metadata/hotel-native-webmcp-reconciliation.json), [`metadata/vercel-hotel-deployment.json`](metadata/vercel-hotel-deployment.json), [`metadata/demo-video-production.json`](metadata/demo-video-production.json), and [`docs/23-hotel-booking-demo.ja.md`](docs/23-hotel-booking-demo.ja.md).
