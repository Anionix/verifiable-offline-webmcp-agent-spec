...
This local reference implementation prevents a second visible notification for the same logical operation across retries, response loss, and process restarts. Simulated delivery is verified. A real notification is not attempted until the user reviews its content and target and gives immediate approval.
