...
observed_at: "2026-08-30T13:56:24.118Z"
status: "sites-version-12-historical-vercel-native-webmcp-release-current-devpost-version-12-current"
---
...
ChatGPT Sites版12の画面には、端末内履歴から読む検証パネルがあり、同じ予約条件を表すUUIDv5、履歴件数、最新イベントのUUIDv7、SHA-256連鎖の先頭12文字と連鎖検査結果を表示します。Sites版の過去の実操作証拠は、同じ機能要約値を持つ版10から版12へ引き継いだものです。現在の主たるネイティブWebMCP証拠は、別の公開先であるVercel本番版です。この表示と記録は端末内履歴の可視化であり、実在する外部予約の証明ではありません。
...
| ChatGPT Sites一般公開 | 成功（現行版） | [本番URL](https://kyoto-booking-retry-proof.anionix.chatgpt.site)の版12は、main履歴へ到達可能な提供元コミット`f832cc611ed43613035a8735ca97d4bc1a0a8efc`へ結び付けて記録する。生成物の完全パッケージ要約値も再現境界として保持し、配置`appgdep_6a92e425bbdc8191ac9ca35d524f61a0`の匿名HTTP 200と四つの静的ファイル一致を確認。再送収束は同じ機能要約値の版10から引き継ぎ、版12で再実行したとは扱わない |
| 可視証拠パネル | 成功（現行機能） | 版10でUUIDv5`64ccc2dc-0404-5566-b296-92d0eb7ed00f`、最新UUIDv7、履歴4件、SHA-256連鎖`Valid`を確認。版11で同じ結果を復元 |
| Vercel一般公開 | 成功（現行版） | 本番配置`dpl_AdzeHw7CgM3sbsZBVutZZbLLbeAK`はソースコミット`c8be388d8047472ef7d6ad69656255adb5903e37`へ結び付いて`READY`。匿名取得した評価ファイルは手元の要約値と一致し、192件のNode試験を報告。対応するChrome実測では四機能、発見時の作用ゼロ、状態確認後の`RETRY_RECOGNIZED`、試行2回・予約1件・処理開始1回・同じ確認番号を確認 |
...
- 安全版依存を使う機能コミット`5ac1fe51a29800eb052f9a63e7311559b7c01e45`は、ChatGPT Sites版10でWebMCP四機能、人間確認境界、2試行から1予約・処理開始1回への収束を実測しました。最終状態を含む提供元報告コミット`f832cc611ed43613035a8735ca97d4bc1a0a8efc`はmain履歴へ到達可能な固定ソースとして保持し、版12の完全パッケージ要約値を生成物の再現境界として、版識別子`appgprj_6a923239002081918896546134a7dc8f~appgver_feeff1aa402c8191a02e0945d9cdd04e`、配置`appgdep_6a92e425bbdc8191ac9ca35d524f61a0`から同じURLへ一般公開済みです。匿名HTTP 200と四つの静的ファイル一致を確認しました。版12の機能要約値は版10と同一なので実操作証拠を引き継ぎますが、版12で再実行したとは主張しません。
- Vercelの現在の本番配置`dpl_AdzeHw7CgM3sbsZBVutZZbLLbeAK`はソースコミット`c8be388d8047472ef7d6ad69656255adb5903e37`へ結び付き、`2026-08-30T13:47:14.754Z`に`READY`となりました。[一般公開URL](https://kyoto-booking-retry-proof.vercel.app)と[配置固有URL](https://kyoto-booking-retry-proof-jnkzuh7r0-aniotajp-1978s-projects.vercel.app)から匿名で取得した評価ファイルは手元の要約値と一致し、192件のNode試験を報告します。対応するChrome実測では`document.modelContext`、意図した四機能、発見時の作用ゼロ、状態確認後の再送収束を確認しました。旧配置`dpl_ArJPwr1h3KqyxmRRfegcbX4YqTB2`の記録は履歴として残し、現在の実測と混ぜません。誤って対象にした旧通知プロジェクトは配置`dpl_3KTHTtZ5h8quDhviMTRo5GxBuUuE`へ復旧済みで、現在も別プロジェクトです。
...
- Devpostの[一般プロジェクトページ](https://devpost.com/software/project-y79pb23hj1mz)は、画面の題名と見出しを`Kyoto Booking Retry Proof`にし、現在は192件のNode試験、現行Vercel配置、四機能のネイティブ発見、状態確認後の安全な再送を反映しています。版8で説明、版9で技術・リンク・動画、版10で冒頭の式・見出し・再送結果リスト、版11で60秒の四手順、版12で現行の公開証拠を整えました。提出後の匿名HTMLでは`og:title`も`Kyoto Booking Retry Proof`、`og:image`も[アップロード済みホテル画像](https://d112y698adiu2z.cloudfront.net/photos/production/software_thumbnail_photos/005/194/459/datas/medium.png)になりました。提出番号`1158722`は`Submitted`で、送信応答と認証済みプロジェクト読み戻しの`submitted_at`は`2026-08-29T09:14:00.129-04:00`で一致しました。過去の153件スナップショットは[`metadata/devpost-public-readback.json`](../metadata/devpost-public-readback.json)に履歴として保存しています。
