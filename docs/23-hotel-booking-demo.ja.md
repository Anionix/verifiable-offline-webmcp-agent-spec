...
| Vercel一般公開 | 成功（現行版） | ソース`037496f6db7281b7b1a9ecd9b3dfc71d407feeb6`の配置`dpl_8kb6oNsU2dFwEu997zbj4zpeuWSL`を[ホテル専用URL](https://kyoto-booking-retry-proof.vercel.app)と[配置固有URL](https://kyoto-booking-retry-proof-4p3pru70s-aniotajp-1978s-projects.vercel.app)で匿名確認。5ファイル一致、警告・エラー0件。再送収束は同じ機能要約値の直前配置`dpl_4uthDyjgSi1KxbssW9t5u18xJbLs`から引き継ぎ、現行配置で再実行したとは扱わない |
