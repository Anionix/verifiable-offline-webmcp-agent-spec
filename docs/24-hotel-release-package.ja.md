...
<!-- event_uuid_v7=01a0576f-4c85-7731-b4b3-3833d8af4a2f state_transition=REPRODUCIBLE_RELEASE_CONTRACT_IMPLEMENTED -> PUBLIC_TARGETS_AND_DESCRIPTION_ALIGNED occurred_at=2026-08-31T10:48:27.013Z machine-contract=Current Sites, Vercel, and Devpost alignment is recorded separately from the package build. -->
...
現在の公開版対応は、提出先ChatGPT Sites版14（ソース`2fbbf1b714ca660ef1681239b638205a9835f7c5`）、Vercel本番配置`dpl_HWJVg4uCgFEaq9N2f5kvXwLjvK2E`（ソース`2d5abd679893ec7dff36758925477999424c3cc7`）、Devpost版16を別々に読み戻した[`公開版対応表`](../metadata/public-release-alignment-readback.json)で確認します。三つの評価は194件と四機能で一致します。ネイティブWebMCPは公開HTTPS別名で実行し、結果は別の実測記録に保存しています。配置固有URLは分離ブラウザーで読み戻しましたがサインインへ転送されたため、実行は一般公開URLで行いました。外部効果は測定していません。
