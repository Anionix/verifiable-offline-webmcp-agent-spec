...
3. 同じ日付・人数・部屋数で**Retry the same booking**を押す。
4. `RETRY_RECOGNIZED`、試行2回、予約1件、効果開始1回、同じ確認番号を指す。
