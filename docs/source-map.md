...
Current public alignment / 現在の公開版対応: the [alignment readback](../metadata/public-release-alignment-readback.json), observed `2026-08-31T10:48:27.013Z`, separately reads submitted ChatGPT Sites version 14, Vercel deployment `dpl_HWJVg4uCgFEaq9N2f5kvXwLjvK2E` from source `2d5abd679893ec7dff36758925477999424c3cc7`, and Devpost version 16. The three observations agree on the submitted Site URL, the 194-test release count, and the exact four-tool contract. A fresh native browser run is recorded separately against the canonical public HTTPS alias after READY; its result fields do not measure page network traffic or external effects. / 現在の対応表は提出先、Vercel、Devpostを別々に読み戻し、194件と4機能の一致を示します。新しいネイティブ実行は公開HTTPS別名で別記録に保存し、ページ通信や外部効果は測定していません。
