...
public_release_alignment_observed_at: "2026-08-31T09:54:07.000Z"
version: "0.1.0"
...
Current public alignment / 現在の公開版対応: the [alignment readback](../metadata/public-release-alignment-readback.json), observed `2026-08-31T09:54:07.000Z`, separately reads submitted ChatGPT Sites version 14, Vercel deployment `dpl_BetejQ7wQwwouGcyC6HW588EiqK8`, and Devpost version 14. The three observations agree on the submitted Site URL, the 194-test release count, and the exact four-tool contract. A fresh native browser run is recorded separately against a unique alias of the same Vercel deployment; its result fields do not measure page network traffic or external effects. / 現在の対応表は提出先、Vercel、Devpostを別々に読み戻し、194件と4機能の一致を示します。新しいネイティブ実行は同じVercel配置の一意な別名で別記録に保存し、ページ通信や外部効果は測定していません。
