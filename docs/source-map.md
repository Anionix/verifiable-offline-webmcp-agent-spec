...
Historical provenance / 過去の出所: this document uses repository base `5583cdbeddbbeae2c6f16fd481fc809069a15296`. The [hotel verification record](../metadata/hotel-booking-verification.json) is an overall historical record with record time `2026-08-29T13:52:59.000Z` and Sites 12; that version label does not replace the versions of its individual observations. Direct runtime observations remain: `6f65cf02-40eb-51c3-bdbc-4d532082fe0e` on Chrome against Sites 6, `311a96f7-7859-5b7f-baf2-17e39b83502d` on Sites 10 and Vercel deployment `dpl_4uthDyjgSi1KxbssW9t5u18xJbLs`, and `e72277c5-f851-57da-a456-bfffc43f9479` on Sites 10. Later Sites 12 and the older Vercel entries are static/public-reachability or digest confirmations only where the rows say so; no aggregate `updated_at` is assigned as an individual observation time. The [native reconciliation record](../metadata/hotel-native-webmcp-reconciliation.json) and [public release readback](../metadata/hotel-public-release-readback.json) use source `c8be388d8047472ef7d6ad69656255adb5903e37`; their individual observations are `2026-08-30T13:53:37.248Z` and `2026-08-30T13:56:24.118Z` at [Vercel](https://kyoto-booking-retry-proof.vercel.app/) and its [evaluation file](https://kyoto-booking-retry-proof.vercel.app/webmcp-evals.json). The `c8be...` observation is not rewritten as a `5583...` execution. / この文書の基準は`5583...`です。全体記録の版と個別観測の版・時刻を混同せず、SitesとVercelの記録は元の時刻とsourceを保ちます。

Current public alignment / 現在の公開版対応: the [alignment readback](../metadata/public-release-alignment-readback.json), observed `2026-08-31T10:18:23.000Z`, separately reads submitted ChatGPT Sites version 14, Vercel deployment `dpl_A39LNXnMRAA6RscBYJLkZBok1Y3B` from source `bbb1b611dbaf9bb2172e59da3e63bbe71799bfeb`, and Devpost version 15. The three observations agree on the submitted Site URL, the 194-test release count, and the exact four-tool contract. A fresh native browser run is recorded separately against a unique alias of the same Vercel deployment after READY; its result fields do not measure page network traffic or external effects. / 現在の対応表は提出先、Vercel、Devpostを別々に読み戻し、194件と4機能の一致を示します。新しいネイティブ実行は同じVercel配置の一意な別名で別記録に保存し、ページ通信や外部効果は測定していません。
...
| Read order / 読む順番 | General requirement / 一般要件 | General test / 一般試験 | Hotel evidence / ホテル証拠 | Status / 状態 | Direct runtime, static check, and record / 直接実行・静的照合・記録 |
|---|---|---|---|---|---|
| 1. Read status first / 先に状態を読む | `REQ-EXEC-002`<br>不明な結果から変更操作へ直接再送しない / Do not retry a change directly from an unknown result. | `TEST-MODEL-002`<br>一般の有限状態モデル試験 / General finite-state model test. | Native reconciliation: status read found `COMMITTED`, then retry reached `RETRY_RECOGNIZED`; human confirmation only. | `PASS` in the native record. | Direct: Vercel native reconciliation; observed `2026-08-30T13:53:37.248Z`. |
| 2. Keep one effect / 効果を一回にする | `REQ-EXEC-003`<br>同じ意思の効果は高々1回 / The same intent starts an effect at most once. | `TEST-MODEL-003`<br>一般の有限状態モデル試験 / General finite-state model test. | `311a96f7-7859-5b7f-baf2-17e39b83502d`: direct Sites 10 and Vercel runs reported 2 attempts → 1 booking. Sites 12 and current Vercel retained the same functional digest and anonymously fetched static-file match. | `PASS` | Direct: Sites 10 + Vercel `dpl_4uthDyjgSi1KxbssW9t5u18xJbLs` (time not recorded). Static: Sites 12 + current Vercel. Record: overall Sites 12, record time `2026-08-29T13:52:59.000Z`. |
| 3. Read back / 読み戻す | `REQ-TOOL-003`<br>書き込みの結果は読み戻して確かめる / Check a write result by reading it back. | `TEST-VERIFY-001`<br>一般の通知デモ向け試験 / General notification-demo test. | `6f65cf02-40eb-51c3-bdbc-4d532082fe0e`: Chrome directly observed public Sites 6 with `EMPTY`, the ordinary-versus-WebMCP retry comparison, and four safe tool cards; `document.modelContext` was undefined. The overall Sites 12 record retains this Sites 6 observation; this check was not rerun. | Hotel verification: `INCONCLUSIVE`.<br>Native record: `PASS`. | Direct: Sites 6 (time not recorded). Static: not claimed. Record: overall Sites 12, record time `2026-08-29T13:52:59.000Z`; the Sites 6 observation is retained. |
| 4. Separate claim and observation / 主張と観測を分ける | `REQ-AUDIT-010`<br>記録された主張と独立した観測を分ける / Separate a recorded claim from an independent observation. | `TEST-VERIFY-003`<br>一般の証拠分離試験 / General claim-versus-observation test. | `e72277c5-f851-57da-a456-bfffc43f9479`: direct Sites 10 run proved the four-tool and retry flow; Sites 12 later supplied anonymous static/public reachability and digest confirmation, not a new runtime run. | `PASS` (related hotel evidence). | Direct: Sites 10 (time not recorded). Static: Sites 12. Record: overall Sites 12, record time `2026-08-29T13:52:59.000Z`. |
| 5. Find the matching files / 対応する資料を見つける | `REQ-KNOW-003`<br>配布物と目録を検索できる / Make distributions and manifests discoverable. | `TEST-KNOW-003`<br>一般の台帳整合検査 / General catalog integrity check. | [File catalog / ファイル目録](../metadata/file-catalog.json) links the hotel records above; [the validator / 検査](../scripts/validate_repo.py) checks the general ledger. | General check implemented; public execution is not asserted. / 一般検査は実装済み。公開実行の証明ではありません。 | Catalog of this checkout; no runtime observation or aggregate timestamp is assigned. / この作業版の目録。実行時刻は割り当てません。 |
