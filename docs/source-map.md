...
Current public alignment / 現在の公開版対応: the [alignment readback](../metadata/public-release-alignment-readback.json), observed `2026-08-31T03:49:55.632Z`, separately reads submitted ChatGPT Sites version 14, Vercel deployment `dpl_FqcLjo1xbpmRBoNsUA6WMny8L1Eu`, and Devpost version 13. The three observations agree on the submitted Site URL, the 194-test release count, and the exact four-tool contract. This record proves anonymous publication readback and authenticated description readback; it does not claim a new native browser run or external effects. / 現在の対応表は提出先、Vercel、Devpostを別々に読み戻し、194件と4機能の一致だけを示します。新しいネイティブ実行や外部効果は主張しません。
