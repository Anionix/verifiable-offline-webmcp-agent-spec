...
event_uuid_v7: "01a04c2d-3874-76d8-a05a-b367169ee25a"
occurred_at: "2026-08-29T06:20:27Z"
state: "FINAL"
state_transition: "NOT_AUTHORED -> SCRIPT_DRAFTED -> AUDIO_GENERATED -> SUBTITLES_AUDIO_TIMED -> VIDEO_ASSEMBLY_PENDING -> PUBLIC_SITE_RECORDED -> FINAL_VIDEO_VERIFIED -> CURRENT_RESULT_ENDING_VERIFIED"
---
...
- Measured actual public-site screen recording: **113.2 seconds (75.5%)**, from 0:20 through 2:13.2. The final 16.8 seconds use current public Sites version 7 captures of the service table and retry result; those still captures are not counted as screen-recording time.
...
| 2:11–2:13.2 | Keep the recorded service table on screen and read its current values. Never replace an unknown value with success. | ChatGPT Sites is the primary home; Vercel is the backup. | 13 |
| 2:13.2–2:22 | Show the current public Sites version 7 service-table capture. | Sites and Vercel are `CURRENT_ARTIFACT` and `CURRENT_ARTIFACT_VERIFIED`; unrun services remain visibly unrun. | 13 |
| 2:22–2:30 | Show the current public Sites version 7 retry-result capture. Keep captions below the result counts. | `RETRY_RECOGNIZED`, attempts `2`, bookings `1`, effect starts `1`; one fictional booking and no real charge. | 14 |
...
- Verified v10: 150 seconds, audible stereo audio, 113.2 seconds of actual public-site screen recording, 75.5 percent actual-site share, current public Sites version 7 service-table and retry-result captures, and the exact generated-scene disclosure throughout the first 20 seconds.
